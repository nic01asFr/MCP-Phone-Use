...
Le téléphone se connecte **en sortant** vers le relais — n'importe quel hébergement HTTPS public convient (VPS, pod cloud, conteneur...), voir [`Dockerfile`](../Dockerfile) et [`relais/README.md`](../relais/README.fr.md). Aucun ADB, aucun NAT à percer, aucun port ouvert côté téléphone. Exemple de déploiement réel utilisé pendant le développement : pod SSPCloud, namespace `nic01asfr` — un choix d'hébergement, pas une contrainte du projet.
